		...
		<div className="min-h-screen bg-gray-50 pb-12 font-sans">
				...
				<div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
					<div className="flex items-center gap-4">
						<div className="flex items-center gap-2">
							...
							<div className="w-9 h-9 bg-primary rounded-xl flex items-center justify-center text-white shadow-md transform transition-transform hover:rotate-3">
								<img src="/logo.png" alt="Logo Align" className="h-5 w-5 object-contain" />
							</div>
							<div>
						...
						<button
							...
							<div className="flex -space-x-3 relative">
								{users.map((u) => (
									<img
										key={u.id}
										className="w-9 h-9 rounded-full border-2 border-white ring-1 ring-gray-200 shadow-sm"
										src={u.avatar}
										alt={u.name}
									/>
								))}
							...
							<div className="text-left">
								<p className="text-sm font-bold text-gray-700 group-hover:text-primary transition-colors leading-tight">
									{profileName}
								</p>
								<p className="text-[10px] text-gray-400 font-medium">Perfil Conjunto</p>
							</div>
							<svg
								className="w-4 h-4 text-gray-300 group-hover:text-primary transition-colors ml-1"
								fill="none"
								stroke="currentColor"
								viewBox="0 0 24 24">
								<path
					...
					<div className="flex items-center gap-3">
						<button
							...
							className="hidden sm:flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold text-primary bg-green-50 hover:bg-green-100 border border-green-100 transition-all">
			...
			<main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
				...
				<div className="sm:hidden mb-6 flex gap-3">
					...
					<button
						onClick={() => setIsAddModalOpen(true)}
						className="flex-[2] flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-bold text-primary bg-green-50 border border-green-100 active:bg-green-200">
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
					...
					<div className="lg:col-span-2 space-y-8">
						...
						<div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
							<div className="bg-gradient-to-br from-primary to-green-600 rounded-2xl p-5 text-white shadow-lg transform hover:scale-[1.02] transition-transform">
								...
								<div className="mt-4 flex items-center text-xs text-green-100 bg-white bg-opacity-20 w-fit px-2 py-1 rounded">
							...
							<div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
								...
								<div className="mt-4 h-1.5 bg-gray-100 rounded-full overflow-hidden">
									<div
										...
										style={{
											width: `${Math.min((currentMonthExpenses / totalIncome) * 100, 100)}%`,
										}}></div>
								</div>
							</div>
							<div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
								...
								{transactions.map((tx) => (
									<div
										...
										<div className="text-right">
											<p
												className={`font-bold ${
													tx.type === TransactionType.INCOME ? 'text-green-600' : 'text-gray-800'
												}`}>
												{tx.type === TransactionType.INCOME ? '+' : '-'} R${' '}
												{tx.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
											</p>
											<div
												className="flex items-center justify-end gap-1 mt-1"
												title={`Responsável: ${users.find((u) => u.id === tx.userId)?.name}`}>
												<img
					...
					<div className="space-y-8">
						...
						<div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
							...
							<p className="text-sm text-gray-500 mb-4">
								Percentual do salário destinado automaticamente para as metas.
							</p>
